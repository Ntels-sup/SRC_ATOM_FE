package com.ntels.avocado.domain.atom.management.trace;

public class TraceResCDR {
	
	private String package_id;
	private String package_name;
	private String service_id;
	private String service_name;
	private String trace_req_id;
	private String result_cdr_id;
	private String result_udr_id;
	private String result_type;
	private String udr_id;
	private String cdr_data;
	
	
	public String getPackage_id() {
		return package_id;
	}
	public void setPackage_id(String package_id) {
		this.package_id = package_id;
	}
	public String getService_id() {
		return service_id;
	}
	public void setService_id(String service_id) {
		this.service_id = service_id;
	}
	public String getPackage_name() {
		return package_name;
	}
	public void setPackage_name(String package_name) {
		this.package_name = package_name;
	}
	public String getService_name() {
		return service_name;
	}
	public void setService_name(String service_name) {
		this.service_name = service_name;
	}
	public String getTrace_req_id() {
		return trace_req_id;
	}
	public void setTrace_req_id(String trace_req_id) {
		this.trace_req_id = trace_req_id;
	}
	public String getResult_cdr_id() {
		return result_cdr_id;
	}
	public void setResult_cdr_id(String result_cdr_id) {
		this.result_cdr_id = result_cdr_id;
	}
	public String getResult_udr_id() {
		return result_udr_id;
	}
	public void setResult_udr_id(String result_udr_id) {
		this.result_udr_id = result_udr_id;
	}
	public String getResult_type() {
		return result_type;
	}
	public void setResult_type(String result_type) {
		this.result_type = result_type;
	}
	public String getUdr_id() {
		return udr_id;
	}
	public void setUdr_id(String udr_id) {
		this.udr_id = udr_id;
	}
	public String getCdr_data() {
		return cdr_data;
	}
	public void setCdr_data(String cdr_data) {
		this.cdr_data = cdr_data;
	}

}
