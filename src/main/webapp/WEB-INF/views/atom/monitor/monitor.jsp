...
<script src="/scripts/custom/amcharts_custom.js"></script>
...
<!--
	 ...
	 */
	function fnInitializeTree(nodeId) {	
		//console.log("트리 nodeId-->"+nodeId);
		var to = false;
		...
		if (nodeId == "") {
			...
			url = "/atom/monitor/listTreeDetail.json?nodeId="+nodeId;
		}
		...
		$("#jstree").bind("select_node.jstree", function (event, data) {
           ...
           console.log(data.node.children.text);
	 ...
	 * 트리 오른쪽 메뉴 
	 */
	function fnCustomMenu(nodeId) {
		//console.log("fnCustomMenu node->"+node);
		console.log("fnCustomMenu nodeId->"+nodeId);
		var items = {
			      "Initialize" : {
			          "label" : "Initialize",
			          "action" : function(obj) {
			        	  fnSendCommand(nodeId, "Initialize");
			        	  return;
			          }
			      },
			      "Run" : {
			      ...
			      "Kill" : {
				         "label" : "Kill",
				         "action" : function(obj) {
				        	 fnSendCommand(nodeId, "Kill");
				        	 return;
				         }
				      }
			   };
		
		if (nodeId == "")  { //컨텍스트 메뉴를 제거한다.
			delete items.Initialize;
			delete items.Run;
			delete items.Stop;
			delete items.Kill;
		}
     ...
     * 프로세스 상태 
     */
	function fnProcessStatus(node) {
		console.log("fnProcessStatus");
		//console.log(node);
	}
	...
	function fnIntervalAlarmSound() {
		intervalAlarmSound = setInterval(function() {
			...
			fnIntervalAlarmSound();
			CommandREQ("0000000009", "", 8, '{"all":true,"process":[]}');
		}, 10 * 1000);
	} 		
	...
	function fnNMConnectOk() {
		fnLoadingTry(false, "");
	}
	
	/** init, run, stop, kill 명령어 날리기 **/ 
	function fnSendCommand(nodeId, command) {
		console.log("fnSendCommand nodeId-->"+nodeId);
		console.log("fnSendCommand command-->"+command);
		if (nodeId != "") {
	...
	function fnCommonMessage(command, messages) {
		...
		if (command == 9) { //alarm clear 응답
			
		} else if (command == 20021) { //alarm clear 응답
	...
	function fnResponseResource(messages) {
		...
		$.each(messages, function(i, row) {
			if (row.NAME == "NIC") {
				$("#nic_name").empty();
				if (row.status == "Y") {
					$("#nic_status").removeClass().addClass("status_type1 normal");
					$("#nic_name").removeClass().addClass("nor");
					$("#nic_name").html("Normal");
				} else {
					$("#nic_status").removeClass().addClass("status_type1 abnormal");
					$("#nic_name").removeClass().addClass("abnor");
					$("#nic_name").html("Abnormal");
				}
			} else {
				$("#pie_name"+t).empty();
				$("#pie_name"+t).append(row.NAME+"<span>"+row.LIST+"</span>");			
				$(tarDiv[t-1]).data('easyPieChart').update(row.usage);
				t += 1;
			}
		});
	}
			...
			result.done(function(data) {
				...
				if (data != null) {
					...
					$.each(data, function(i, row) {
						li = "<li>";
						li += row.resourceDataName+"<ol>";
						
						dataArray = []; 
						$.each(row.resourceData, function(t, row) {
							//console.log(row);
							li +=  "<li>"+row.RSC_ID_NAME+"</li>";
						...
						li +=  "</ol><div id='"+AmChartsColum[i]+"'></div>";
						li +=  "</li>";
						//console.log(li);
	...
	function fnMessageInitAlarm(messages) {
		...
		if (severity_id == 3) $("#alarm_popup .popup_sys .info > div:first").removeClass().addClass("alarm mi"); //색상 class
		if (severity_id == 4) $("#alarm_popup .popup_sys .info > div:first").removeClass().addClass("alarm nor"); //색상 class
		if (severity_id == 5) $("#alarm_popup .popup_sys .info > div:first").removeClass().addClass("alarm cl"); //색상 class
		if (severity_id == 6) $("#alarm_popup .popup_sys .info > div:first").removeClass().addClass("alarm wa"); //색상 class
		if (severity_id == 7) $("#alarm_popup .popup_sys .info > div:first").removeClass().addClass("alarm ind"); //색상 class
...
<script>
	...
	// 배치잡 다이어그램을 토글한다.
	function toggleBatchJobMonitor() {
		var duration = 300;
		if ($("#networkDiagramFrame").css("opacity") == 1) {
			// 노드 상세창이 떠있을 경우 노드 상세창을 숨긴다.
			if ($(".m_submenu").hasClass("on")) {
				$("button.back").trigger("click");
			}
			$("#networkDiagramFrame").animate({
				opacity: 0
			}, {
				duration: duration,
				complete: function() {
					$(this).hide();
					$("#batchJobDiagramFrame").show();
					$("#batchJobDiagramFrame").animate({opacity: 1}, duration);
				}
			});
		}
		if ($("#batchJobDiagramFrame").css("opacity") == 1) {
			$("#batchJobDiagramFrame").animate({
				opacity: 0
			}, {
				duration: duration,
				complete: function() {
					$(this).hide();
					$("#networkDiagramFrame").show();
					$("#networkDiagramFrame").animate({opacity: 1}, duration);
				}
			});
		}
	}
...
<div class="wrap monitoring">
    <div class="m_header"><div class="m_logo">ATOM Monitoring</div>
        ...
        <ul class="m_util">
             <li id="atom_resources" class="util1 dropdown">
                 <a href="#" title="ATOM Info"><span>ATOM Info</span></a>
                       <ul class="dropdown_content">
                             <h6>ATOM System Information</h6>
                             <div id="resource_piechart">
	                             <li>
	                                <div class="pie_graph">
	                                  <span class="pie devchart1" data-percent="0"><span class="percent"></span></span>
	                                  <p id="pie_name1"><span></span></p>
	                                </div>
	                             </li>
	                             <li>
	                                <div class="pie_graph">
	                                  <span class="pie devchart2" data-percent="0"><span class="percent"></span></span>
	                                  <p id="pie_name2"><span></span></p>
	                                </div>
	                             </li>
	                             <li>
	                                <div class="pie_graph">
	                                  <span class="pie devchart3" data-percent="0"><span class="percent"></span></span>
	                                  <p id="pie_name3"><span></span></p>
	                                </div>
	                             </li>
	                             <li>
	                                <div class="pie_graph">
	                                  <span class="pie devchart4" data-percent="0"><span class="percent"></span></span>
	                                  <p id="pie_name4"><span></span></p>
	                                </div>
	                             </li>
	                         </div>
                             <li>
                                <div class="status_info">
                                  <span id="nic_status" class="status_type1 normal">Network</span> <!--class : normal,abnormal--> 
                                  <p>Network<span id="nic_name" class=""></span></p> <!--class : nor,abnor-->
                                </div>
                             </li>
                             <li>
                                <div class="status_info">
                                  <span class="status_type2 normal">Process</span> <!--class : normal,abnormal-->
                                  <p>Process<span class="abnor">Abnormal</span></p> <!--class : nor,abnor-->
                                </div>
                             </li>
                       </ul>
             </li>
             <li id="atom_top_resources" class="util2 dropdown">
             ...
             <li id="atom_batch" class="util3"><a href="javascript:toggleBatchJobMonitor();" title="Batch"><span>Batch</span></a></li>
    ...
    <div class="sub_body">
        ...
        <div class="sub_content change">
        	<!-- <div class="main_content" id="network_diagram"> -->
         	<iframe id="networkDiagramFrame" name="networkDiagramFrame" src="networkDiagram.ajax" style="width:100%;height:100%;border:none;opacity:1;"></iframe> 
         	<iframe id="batchJobDiagramFrame" name="batchJobDiagramFrame" src="batchJobDiagram.ajax" style="width:100%;height:100%;border:none;display:none;opacity:0;"></iframe>
            ...
            <div class="sub_type" style="width:100%;">
				...
				<div><span class="socketTime"></span><h4><spring:message code="label.monitor.temperature.text" /></h4><div id="chart_bar_01"></div></div>
				<div class="data">
				    <table class="tbl_list">
				        ...
				        <thead>
				            <tr>
				                <th><spring:message code="label.monitor.temperature.name" /></th><!-- Name -->
				                <th class="t_r"><spring:message code="label.monitor.temperature.average" /></th><!-- Average -->
				               	<th class="t_r"><spring:message code="label.monitor.temperature.max" /></th><!-- Max -->
				                <th class="t_r"><spring:message code="label.monitor.temperature.min" /></th><!-- Minimum -->
				                <th></th>
				            </tr>
				        </thead>
				    </table>
				    <div class="scroll">
				        <table class="tbl_list">
				            ...
				            <tbody id="temperatureResourceInfo"></tbody>
				        </table>
				    </div>  
				</div>
				</div>
				...
				<div><span class="socketTime"></span><h4><spring:message code="label.monitor.cpu.text" /></h4><div id="chart_pie_01"></div></div>
				<div class="data">
				    <table class="tbl_list">
				        <colgroup>
				        <col width="15%"/>
				        ...
				        <thead>
				            <tr>
				                <th><spring:message code="label.monitor.cpu.name" /></th><!-- Name -->
				                <th class="t_r"><spring:message code="label.monitor.cpu.usage" /></th><!-- Usage -->
				                <th class="t_r"><spring:message code="label.monitor.cpu.user" /></th><!-- User -->
				                <th class="t_r"><spring:message code="label.monitor.cpu.system" /></th><!-- System -->
				                <th class="t_r"><spring:message code="label.monitor.cpu.idle" /></th><!-- Idle -->
				                <th class="t_r"><spring:message code="label.monitor.cpu.wait" /></th><!-- Wait -->
				                <th></th>
				            </tr>
				        </thead>
				    </table>
				    <div class="scroll">
				        <table class="tbl_list">
				            ...
				            <tbody id="cpuResourceInfo"></tbody>
				        </table>
				    </div>                    
				...
				<div><span class="socketTime"></span><h4><spring:message code="label.monitor.disk.text" /></h4><div id="chart_bar_02"></div></div>
				<div class="data">
				    <table class="tbl_list">
				        <colgroup>
				        <col width="15%"/>
				        ...
				        <thead>
				            <tr>
				                <th><spring:message code="label.monitor.cpu.name" /></th><!-- Name -->
				                <th class="t_r"><spring:message code="label.monitor.disk.usage" /></th><!-- Disk Usage<br/>(%) -->
				                <th class="t_r"><spring:message code="label.monitor.disk.total" /></th><!-- Total Space<br/>(MB) -->
				                <th class="t_r"><spring:message code="label.monitor.disk.used" /></th><!-- Used Space<br/>(MB) -->
				                <th class="t_r"><spring:message code="label.monitor.disk.free" /></th><!-- Free -->
				                <th></th>
				            </tr>
				        </thead>
				    </table>
				    <div class="scroll">
				        <table class="tbl_list">
				            ...
				            <tbody id="diskResourceInfo"></tbody>
				        </table>
				    </div> 
				</div>
				<!--// data -->
				...
				<div><span class="socketTime"></span><h4><spring:message code="label.monitor.memory.text" /></h4><div id="chart_pie_02"></div></div>
				<div class="data">
				    <table class="tbl_list">
				        <colgroup>
				        <col width="15%"/>
				        ...
				        <thead>
				            <tr>
				                <th><spring:message code="label.monitor.cpu.name" /></th><!-- Name -->
				                <th class="t_r"><spring:message code="label.monitor.memory.usage" /></th><!-- Memory Usage<br/>(%) -->
				                <th class="t_r"><spring:message code="label.monitor.disk.total" /></th><!-- Total Space<br/>(MB) -->
				                <th class="t_r"><spring:message code="label.monitor.disk.used" /></th><!-- Used Space<br/>(MB) -->
				                <th class="t_r"><spring:message code="label.monitor.disk.free" /></th><!-- Free -->
				                <th></th>
				    ...
				    <div class="scroll">
				        <table class="tbl_list">
				            ...
				            <tbody id="memoryResourceInfo"></tbody>
				        </table>
				    </div> 
				</div>
				<!--// data -->
				...
				<div><span class="socketTime"></span><h4><spring:message code="label.monitor.queue.text" /></h4><div id="chart_bar_03"></div></div>
				<div class="data">
				    <table class="tbl_list">
				        <colgroup>
				        <col width="35%"/>
				        ...
				        <thead>
				            <tr>
				                <th><spring:message code="label.monitor.cpu.name" /></th><!-- Name -->
				                <th class="t_r"><spring:message code="label.monitor.queue.usage" /></th><!-- Queue Usage<br/>(%) -->
				                <th></th>
				            </tr>
				        </thead>
				    </table>
				    <div class="scroll">
				        <table class="tbl_list">
				            ...
				            <tbody id="queueResourceInfo"></tbody>
				        </table>
				    </div> 
				</div>
				<!--// data -->
				...
				<div><span class="socketTime"></span><h4><spring:message code="label.monitor.tablespace.text" /></h4><div id="chart_pie_03"></div></div>
				<div class="data">
				    <table class="tbl_list">
				        <colgroup>
				        <col width="25%"/>
				        ...
				        <thead>
				            <tr>
				                <th><spring:message code="label.monitor.cpu.name" /></th><!-- Name -->
				                <th class="t_r"><spring:message code="label.monitor.tablespace.usage" /></th><!-- Usage(%) -->
				                <th class="t_r"><spring:message code="label.monitor.disk.total" /></th><!-- Total Space<br/>(MB) -->
				                <th class="t_r"><spring:message code="label.monitor.disk.used" /></th><!-- Used Space<br/>(MB) -->
				                <th class="t_r"><spring:message code="label.monitor.disk.free" /></th><!-- Free -->
				                <th></th>
				    ...
				    <div class="scroll">
				        <table class="tbl_list">
				            ...
				            <tbody id="tablespaceResourceInfo"></tbody>
				        </table>
				    </div> 
				</div>
				<!--// data -->
				...
				<div><span class="socketTime"></span><h4><spring:message code="label.monitor.nic.text" /></h4><div id="chart_cluster_01"></div></div>
				<div class="data">
				    <table class="tbl_list">
				        <colgroup>
				        <col width="15%"/>
				        ...
				        <thead>
				            <tr>
				                <th><spring:message code="label.monitor.cpu.name" /></th><!-- Name -->
				                <th><spring:message code="label.monitor.nic.status" /></th><!-- Status -->
				                <th class="t_r"><spring:message code="label.monitor.nic.tx" /></th><!-- TX -->
				                <th class="t_r"><spring:message code="label.monitor.nic.rx" /></th><!-- RX -->
				                <th></th>
				            </tr>
				        </thead>
				    </table>
				    <div class="scroll">
				        <table class="tbl_list">
				            ...
				            <tbody id="nicResourceInfo"></tbody>
				        </table>
				    </div> 
				</div>
				<!--// data -->
	...
	<input type="" name="packageName" value="${arrayPackage}" /><!-- 패키지 권한 -->
	<input type="" name="nodeId" value="" /><!-- 패키지 권한 -->
...
<script>
	...
	/* sub page */
	$("button.back").click(function(){ /*.m_logo를 노드 클릭으로 변경해야 합니다.*/
	...
	/* tab */
	$(".tab1").click(function(){ //.tab 클릭시 sect type2의 page2~8 까지 호출
		
		console.log( "프로세스 플로우" );
